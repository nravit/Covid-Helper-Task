...
class BaseAPI: NSObject {
    ...
    func checkForValidation(withData data: Any? , completion: FinalSuccess ) -> Void {
        if  (((data as? [String:Any])?["Status"] as? String) ?? "Failure") == "Success" {
            completion(true , (((data as? [String:Any])?["message"] as? String) ?? "Success"))
        } else {
            completion(false , (((data as? [String:Any])?["message"] as? String) ?? "Messages.WENT_WRONG"))
        }
    }
    ...
    func hitDeleteApi(requests : Request , completion : @escaping BaseAPICompletion ) {
         
         // for alamofire v 5.0.0
        do {
            guard let url = URL(string: requests.url) else { return }
            
            var request = URLRequest(url: url)
            request.httpMethod = "DELETE"
            request.headers = requests.headers ?? [:]
            //request.addValue("application/json", forHTTPHeaderField: "Content-Type")
            guard let httpBody = try? JSONSerialization.data(withJSONObject:requests.parameters ?? [:], options: []) else { return }
            request.httpBody = httpBody
            self.task = AF.request(request)
